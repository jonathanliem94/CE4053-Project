...
{
    struct os_avl_node *aa, *bb;
    aa = _get_entry(a, struct os_avl_node, avl);
    bb = _get_entry(b, struct os_avl_node, avl);

    if (aa->deadline < bb->deadline) return -1;
    else if (aa->deadline > bb->deadline) return 1;
    else return 0;
}
