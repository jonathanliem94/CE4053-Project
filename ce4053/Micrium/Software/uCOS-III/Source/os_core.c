...
void  OSIntExit (void)
{
    ...
    OSPrioHighRdy   = OS_PrioGetHighest(OS_PrioAVL_root);                  /* Find highest priority                                  */
...
void  OSSched (void)
{
    ...
    CPU_INT_DIS();
    OSPrioHighRdy   = OS_PrioGetHighest(OS_PrioAVL_root);                  /* Find the highest priority ready                        */
...
void  OSStart (OS_ERR  *p_err)
{
    ...
    if (OSRunning == OS_STATE_OS_STOPPED) {
        OSPrioHighRdy   = OS_PrioGetHighest(OS_PrioAVL_root);              /* Find the highest priority                              */
...
{
    OS_PrioInsert(OS_PrioAVL_root,p_tcb->Prio);
...
void  OS_RdyListRemove (OS_TCB *p_tcb)
{
    ...
    if (p_tcb1 == (OS_TCB *)0) {                            /* Was the OS_TCB to remove was at the head?              */
        if (p_tcb2 == (OS_TCB *)0) {                        /* Yes, was it the only OS_TCB?                           */
            ...
            p_rdy_list->HeadPtr    = (OS_TCB   *)0;
            p_rdy_list->TailPtr    = (OS_TCB   *)0;
            OS_PrioRemove(OS_PrioAVL_root,p_tcb->Prio);
        } else {
